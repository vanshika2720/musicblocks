...
A generated candidate is a starting point, not the last step, regardless of
which kind of module it targets:

- With no model-backed provider configured, `--generate`/`--emit` produce a
  `NoopClient` placeholder (`it.todo` stubs). The validator correctly rejects
  it as containing no meaningful assertion — that rejection is expected, not
  a bug, and is evidence the safety layer works, not evidence the module was
  a bad choice.
